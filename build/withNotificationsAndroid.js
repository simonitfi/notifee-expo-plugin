...
const { Colors, Permissions } = config_plugins_1.AndroidConfig;
...
const withNotificationManifest = (config, { icon, color, types }) => {
    // If no icon or color provided in the config plugin props, fallback to value from app.json
    icon = icon || getNotificationIcon(config);
    color = color || getNotificationColor(config);
    return (0, config_plugins_1.withAndroidManifest)(config, (config) => {
        config.modResults = setNotificationConfig({ icon, color, types }, config.modResults);
        if (config.modResults.manifest.application) {
            if (!config.modResults.manifest.application[0].service) {
                ...
                if (config.modResults.manifest.application && config.modResults.manifest.application[0].service) {
                    const services = config.modResults.manifest.application[0].service;
                    const nameToRemove = "app.notifee.core.ForegroundService";

                    // Remove the item with a matching name
                    config.modResults.manifest.application[0].service = services.filter(service => {
                        const serviceName = service.$['android:name'];
                        return serviceName !== nameToRemove;
                    });
                }
            }
...
function setNotificationSounds(projectRoot, sounds) {
    ...
    for (const soundFileRelativePath of sounds) {
        //Allow only suported extenssions
        // Extract the file extension from the relative path
        const fileExtension = soundFileRelativePath.split('.').pop().toLowerCase();

        // Check if the file extension matches one of the supported formats
        const isSupportedFormat = ['mp3', 'ogg', 'wav'].includes(fileExtension);
...
const withNotificationsAndroid = (config, { icon = null, color = null, types = null, sounds = [] }) => {
    config = (0, exports.withNotificationIconColor)(config, { color });
    config = (0, exports.withNotificationIcons)(config, { icon });
    config = (0, exports.withNotificationManifest)(config, { icon, color, types });
    config = (0, exports.withNotificationSounds)(config, { sounds });
    config = Permissions.withPermissions(config, [
